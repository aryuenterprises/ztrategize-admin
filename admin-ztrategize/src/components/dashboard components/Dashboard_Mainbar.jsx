                ...
                <div className="flex flex-col w-full mt-5 h-[210px] rounded-2xl bg-[url('././assets/zigzaglines_large.svg')] bg-cover shadow-lg px-4 py-5 md:px-6 md:py-6">
                  ...
                  <div className="max-h-52 overflow-y-auto pr-1.5 space-y-3 custom-scrollbar">
                    {pendingRequestList && pendingRequestList.length > 0 ? (
                      pendingRequestList
                        .slice(-3) // ✅ Only take the last 3 requests
                        .reverse() // ✅ Show newest first
                        .map((req) => {
                          // Format date safely
                          const formattedDate = new Date(req.createdAt).toLocaleDateString("en-GB", {
                            day: "2-digit",
                            month: "short",
                            year: "numeric",
                          });
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="text-sm">
                                <p className="font-medium text-gray-800">
                                  {req.employeeId?.employeeName || "Unknown Employee"}
                                </p>
                                <p className="text-xs text-gray-400">{formattedDate}</p>
                              </div>

                              {/* Right side - Leave type/status badge */}
                              <span
                                className={`px-2 py-0.5 rounded-full text-xs font-semibold ${req.status === "approved"
                                    ? "bg-green-100 text-green-700"
                                    : req.status === "pending"
                                      ? "bg-yellow-100 text-yellow-700"
                                      : "bg-red-100 text-red-700"
                              ...
                              >
                                {req.leaveType?.toUpperCase() || "UNKNOWN"}
                              </span>
                            </div>
                          );
                        })
