...
const Expense_icome_details = () => {
  ...
  const openEditModal = (row) => {
    ...
    setAmountedit(row.debit_amount);
  ...
  // Temporary values before clicking submit
  ...
  const filteredChartData = useMemo(() => {
    ...
    const monthlyTotals = filteredData.reduce((acc, row) => {
      ...
      acc[month] = (acc[month] || 0) + parseFloat(row.debit_amount || 0);
      return acc;
    }, {});
  ...
  const filteredChartDataYear = useMemo(() => {
    if (!filteredData) return null;

    const yearTotals = filteredData.reduce((acc, row) => {
      const year = new Date(row.date).getFullYear();
      acc[year] = (acc[year] || 0) + parseFloat(row.debit_amount || 0);
      return acc;
    }, {});
  ...
  useEffect(() => {
    if (!filteredChartDataYear) return;
    ...
    chartInstance1.current = new Chart(ctx, {
      type: "line",
      data: {
        labels: filteredChartDataYear.labels,
        datasets: [
          {
            label: "Expense",
            data: filteredChartDataYear.values,
            borderColor: "orange",
            borderWidth: 2,
            fill: false,
          },
...
export default Expense_icome_details;
